...
class ConvolutionSmoothing(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, kernel_size: int, keep_dims: bool=False):
        self.kernel_size = check_scalar(kernel_size, 
                                        name="kernel_size",
                                        min_val=2,
                                        target_type=int,
                                        )
    ...
    def transform(self, X):
        ...
        return np.apply_along_axis(self._smooth_data, axis=1, arr=X, **{"kernel_size": self.kernel_size,
...
class SavGol(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        """
        return savgol_filter(X, window_length=self.window_length, polyorder=self.polyorder, deriv=self.deriv)
